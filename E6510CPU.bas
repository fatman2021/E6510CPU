...
dim shared as string driverName

type MEMORY_T
  public:
  declare constructor
  declare destructor
  declare function ReadByte   (byval adr as uinteger) as  byte
  declare function ReadUByte  (byval adr as uinteger) as ubyte
  declare function ReadUInt (byval adr as uinteger) as uinteger
  declare sub      WriteByte  (byval adr as uinteger, byval b8  as   byte)
  declare sub      WriteUByte (byval adr as uinteger, byval b8  as  ubyte)
  declare sub      WriteUInt(byval adr as uinteger, byval w16 as uinteger)
  ...
  const as integer flopy_end  = &HDFFF '------+-|
  const as integer flopy_base = &HDF00 ' 256 b  |
  const as integer cpm_end    = &HDEFF '--------|
  const as integer cpm_base   = &HDE00 ' 256 b  |
  const as integer cia2_end   = &HDDFF '--------|
  const as integer cia2_base  = &HDD00 ' 256 b  |
  const as integer cia1_end   = &HDCFF '--------|
  const as integer cia1_base  = &HDC00 ' 256 b  |-- 4 K I/O
  const as integer col_end    = &HDBFF '--------|
  const as integer col_base   = &HD800 '  1 K   |
  const as integer sid_end    = &HD7FF '--------|
  const as integer sid_base   = &HD400 '  1 K   |
  const as integer vic_end    = &HD3FF '--------|
  const as integer vic_base   = &HD000 '  1 K   |
  const as integer up_ram_end = &HCFFF '------+-|
  const as integer up_ram_base= &HC000 '  4 K |
  const as integer basic_end  = &HBFFF '------|
  const as integer basic_base = &HA000 '  8 K
#endif
                   
  as any ptr  vram  (&H00FFFFFF) ' Video Ram
  as ubyte    ram   (&H10FFFFFF) ' System Ram
...
type CPU6510
  ...
  declare function ADR_IMM   as uinteger
  declare function ADR_REL   as uinteger
  declare function ADR_ZERO  as uinteger
  declare function ADR_ZEROX as uinteger
  declare function ADR_ZEROY as uinteger
  declare function ADR_ABS   as uinteger
  declare function ADR_ABSX  as uinteger
  declare function ADR_ABSY  as uinteger
  declare function ADR_IND   as uinteger
...
function MEMORY_T.Peek64(byval adr as integer) as uinteger
  select case adr
  case &H00E000 to &H00FFFF:return kernal(adr-&HE000)
  case &H00A000 to &H00BFFF:return basic (adr-&HA000)
  case &H00D800 to &H00DBFF:return col   (adr-&HD800)
  case &H00D000 to &H00D3FF
...
sub MEMORY_T.Poke64(byval adr as integer,byval v as uinteger)
  ...
  select case adr
    case 1024 to 2023
      ...
      case &H4000 ' BIOS
        dim as integer dest, source, segment, offset, size, frame_no, tran_color
		select case v
			case &H01 ' Memcopy source, dest, segment, size
			  source  = ram(adr + 1) * &H10000 + ram(adr + 2) * &H100 + ram(adr + 3)
			  dest    = ram(adr + 4) * &H10000 + ram(adr + 5) * &H100 + ram(adr + 6)
			  segment = ram(adr + 7) * &H10000 + ram(adr + 8) * &H100 + ram(adr + 9)
			  size    = ram(adr + 7)
			  for p as integer = 0 to (size - 1)
			    ram(((segment * 16) + source) + p) = ram(((segment * 16) + dest) + p)
			  next
			  for offset = adr + 1 to adr + 16: ram(offset)=0:next 
			case &H02 ' Screen mode [, [ depth ] [, [ num_pages ] [, [ flags ] [, [ refresh_rate ]]]]]
			 ...
			 for offset = adr + 1 to adr + 5: ram(offset)=0:next
			case &H03 ' Screen , [ active_page ] [, [ visible_page ]]
			 screen , ram(adr + 1), ram(adr + 2)
			 ram(adr + 1) = 0: ram(adr + 2) = 0
			case &H04 ' Screenres ( byval width as long, byval height as long, byval depth as long = 8,
			          '             byval num_pages as long = 1, byval flags as long = 0, 
			          '             byval refresh_rate as long = 0 ) as long
			 screenres  ram(adr +  1) * &H1000000 + ram(adr +  2) * &H10000 + ram(adr +  3) * &H100 + ram(adr +  4), _ ' width
			            ram(adr +  5) * &H1000000 + ram(adr +  6) * &H10000 + ram(adr +  7) * &H100 + ram(adr +  8), _ ' heigh
			            ram(adr +  9) * &H1000000 + ram(adr + 10) * &H10000 + ram(adr + 11) * &H100 + ram(adr + 12), _ ' depth
             ...
             next
			 for offset = adr + 1 to adr + 33: ram(offset)=0:next
			case &H05 ' DisplayChar x, y, stringData color
			 ...
			 dim as integer b, xs, ys, clr
			 dim as string strData
             ...
             for offset = adr + 1 to adr + 13: ram(offset)=0:next
            case &H06 ' Cls ( byval mode as long = 1 )
             cls ram(adr + 1): ram(adr + 1) = 0  
            case &H07 ' WaitKey ascii_code
             do until inkey$ = chr$(ram(adr+1)): loop: ram(adr + 1) = 0
            case &H08 ' Sleep ( byval amount as long , byval keyflag as long ) as long
             sleep ram(adr +  1) * &H1000000 + ram(adr +  2) * &H10000 + ram(adr +  3) * &H100 + ram(adr +  4), _ ' amount
			       ram(adr +  5) * &H1000000 + ram(adr +  6) * &H10000 + ram(adr +  7) * &H100 + ram(adr +  8)    ' keyflag
			 for offset = adr + 1 to adr + 8: ram(offset)=0:next      
		    ...
		    case &H0C ' PSet [target ,] [STEP] (x, y) [,color]
		     ...
		     for offset = adr + 1 to adr + 15: ram(offset)=0:next
		    case &H0D ' Put [target, ] [ [STEP](x, y), source [, (x1, y1)-[STEP](x2, y2) ] [, method [, ( alphaval|value|blender [, param]) ] ]     		          	                                          
		     dim as integer xs, ys, target, source, method, value
...
function MEMORY_T.ReadUByte(byval adr as uinteger) as ubyte
  return Peek64(adr)
end function

function MEMORY_T.ReadByte(byval adr as uinteger) as byte
  return Peek64(adr)
end function

function MEMORY_T.ReadUInt(byval adr as uinteger) as uinteger
  return Peek64(adr) or Peek64(adr+1) shl 8
end function

sub MEMORY_T.WriteByte(byval adr as uinteger,byval b8 as byte)
  Poke64(adr,b8)
end sub

sub MEMORY_T.WriteUByte(byval adr as uinteger,byval b8 as ubyte)
  Poke64(adr,b8)
end sub

sub MEMORY_T.WriteUInt(byval adr as uinteger,byval w16 as uinteger)
...
' 6510 address modes
'
function CPU6510.ADR_UNK as uinteger
  #ifdef _DEBUG
  dprint("! adr unknow !")
  beep:sleep:end
  #endif
...
function CPU6510.ADR_IMM as uinteger ' 1 byte #$xx
  ' mem(pc)
  function = PC
  PC+=1
end function

function CPU6510.ADR_REL as uinteger  ' 1 byte (rel. branch -128 - +127)
  function=PC
  PC+=1
end function

function CPU6510.ADR_ABS as uinteger  ' 2 byte $xx:xx
  ' adr = mem(pc) + mem(pc+1)*256
  function = mem->ReadUInt(pc)
  pc+=2
end function

function CPU6510.ADR_ZERO as uinteger ' 1 byte $00:xx
  ' adr = mem(pc) and 255
  function = mem->ReadUByte(pc) and &HFF
  pc+=1
end function

function CPU6510.ADR_ZEROX as uinteger' 1 byte 00:xx,x
  ' adr = (mem(pc)+x) and 255
  function = (mem->ReadUByte(pc)+x) and &HFF
  pc+=1
end function

function CPU6510.ADR_ZEROY as uinteger' 1 byte 00:xx,y
  ' adr = (mem(pc)+y) and 255
  function = (mem->ReadUByte(pc)+y) and &HFF
  pc+=1
end function

function CPU6510.ADR_ABSX as uinteger ' 2 byte $xx:xx,x
  ' adr = mem(pc ) + mem(pc+1)*256 + x
  function  = mem->ReadUInt(PC) + X
  PC+=2
end function

function CPU6510.ADR_ABSY as uinteger ' 2 byte $xx:xx,y
  ' adr = mem(pc ) + mem(pc+1)*256 + y
  function = mem->ReadUInt(PC) + Y
  PC+=2
end function
...
sub INS_BIT(byval Cpu as CPU6510_T)
  dim as byte b
  b=Cpu->mem->Readbyte(Cpu->Code.op.u64)
...
sub INS_CMP(byval Cpu as CPU6510_T)
  dim as MULTI v
  v.u64 = Cpu->A-Cpu->mem->ReadUByte(Cpu->Code.op.u64)
  Cpu->F.c=iif(v.u64<=255,1,0)
  Cpu->F.z=iif(v.ulo =  0,1,0)
  Cpu->F.n=iif(v.slo <  0,1,0)
end sub
...
sub INS_CPX(byval Cpu as CPU6510_T)
  dim as MULTI v
  v.u64 = Cpu->X-Cpu->mem->ReadUByte(Cpu->Code.op.u64)
  Cpu->F.c=iif(v.u64<=255,1,0)
  Cpu->F.z=iif(v.ulo =  0,1,0)
  Cpu->F.n=iif(v.slo <  0,1,0)
end sub
...
sub INS_CPY(byval Cpu as CPU6510_T)
  dim as MULTI v
  v.u64 = Cpu->Y-Cpu->mem->ReadUByte(Cpu->Code.op.u64)
  Cpu->F.c=iif(v.u64<=255,1,0)
  Cpu->F.z=iif(v.ulo =  0,1,0)
  Cpu->F.n=iif(v.slo <  0,1,0)
end sub
...
sub INS_EOR(byval Cpu as CPU6510_T)
  Cpu->A=Cpu->A xor Cpu->mem->ReadUbyte(Cpu->Code.op.u64)
  Cpu->F.z=iif(Cpu->A =0,1,0)
  Cpu->F.n=iif(Cpu->sA<0,1,0)
end sub
...
sub INS_LDA(byval Cpu as CPU6510_T)
  Cpu->A  =Cpu->mem->ReadUbyte(Cpu->Code.op.u64)
  Cpu->F.Z=iif(Cpu->A=0,1,0)
  Cpu->F.N=iif(Cpu->sA<0,1,0)
end sub

sub INS_LDX(byval Cpu as CPU6510_T)
  Cpu->X  =Cpu->mem->ReadUbyte(Cpu->Code.op.u64)
  Cpu->F.Z=iif(Cpu->X=0,1,0)
  Cpu->F.N=iif(Cpu->sX<0,1,0)
end sub

sub INS_LDY(byval Cpu as CPU6510_T)
  Cpu->Y  =Cpu->mem->ReadUbyte(Cpu->Code.op.u64)
  Cpu->F.Z=iif(Cpu->Y =0,1,0)
  Cpu->F.N=iif(Cpu->sY<0,1,0)
end sub
...
sub INS_ORA(byval Cpu as CPU6510_T)
  Cpu->A=Cpu->A or Cpu->mem->ReadUbyte(Cpu->Code.op.u64)
  Cpu->F.z=iif(Cpu->A =0,1,0)
  Cpu->F.n=iif(Cpu->sA<0,1,0)
end sub
...
sub INS_SBC(byval Cpu as CPU6510_T)
  dim as multi v,b
  b.ulo=Cpu->mem->ReadUbyte(Cpu->Code.op.u64)
  v.u64=Cpu->A - b.ulo
  if Cpu->F.c=0 then v.s64-=1
...
sub INS_STA(byval Cpu as CPU6510_T)
  Cpu->mem->WriteUByte(Cpu->code.op.u64,Cpu->A)
end sub

sub INS_STX(byval Cpu as CPU6510_T)
  Cpu->mem->WriteUByte(Cpu->code.op.u64,Cpu->X)
end sub

sub INS_STY(byval Cpu as CPU6510_T)
  Cpu->mem->WriteUByte(Cpu->code.op.u64,Cpu->Y)
end sub
...
data  32,86,50,32,42,42,42,42,13,13,49,54,77,66,32,82
